...
class MCPStack(Stack):
    ...
    def _get_mcp_lambda_code(self, tool_name: str, lambda_path: str, description: str, input_schema: dict) -> str:
        """Generate Lambda function code implementing MCP protocol for AgentCore Gateway"""
        import json as json_module
        
        # Serialize the input schema
        schema_json = json_module.dumps(input_schema)
...
def lambda_handler(event, context):
    """
    MCP Protocol Lambda handler for {tool_name}
    Supports both MCP protocol (tools/list, tools/call) and direct invocation
    """
    ...
    try:
        ...
        if body:
            ...
            mcp_request = json.loads(body) if isinstance(body, str) else body
            ...
            # Handle tools/call
                ...
                tool_name_req = params.get('name')
                arguments = params.get('arguments', {{}})
                
                if tool_name_req != "{tool_name}":
                    return {{
                        'statusCode': 400,
                        'headers': {{'Content-Type': 'application/json'}},
                        'body': json.dumps({{
                            ...
                            'error': {{
                                'code': -32602,
                                'message': f'Unknown tool: {{tool_name_req}}'
                            }},
                            'id': request_id
                        }})
                    }}
                ...
                result = invoke_card_operations(arguments)
        ...
        # Direct invocation (AgentCore Gateway sends arguments directly)
        arguments = event
        result = invoke_card_operations(arguments)
        return result
        ...
        # Return MCP error format if possible
        return {{
            'statusCode': 500,
            'headers': {{'Content-Type': 'application/json'}},
            'body': json.dumps({{
...
def invoke_card_operations(arguments):
    """Invoke the card operations Lambda and return result"""
    card_ops_event = {{
        'httpMethod': 'POST',
        'path': '{lambda_path}',
